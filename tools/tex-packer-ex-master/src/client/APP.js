...
class APP {
    ...
    async doExport() {
        ...
        if (this.packOptions.exporter.type === "Spritemap (Adobe Animate)" && this.packOptions.animationJsonContent) {
            let animName = this.packOptions.animationJsonName || "Animation.json";
            let animContent = this.packOptions.animationJsonContent;
            try {
                let animJson = JSON.parse(animContent);
                let newImgName = textureName + (this.packResult.length > 1 ? "-0" : "") + "." + this.packOptions.textureFormat;
                
                if (animJson.metadata && typeof animJson.metadata === 'object') {
                    animJson.metadata.image = newImgName;
                }
                if (Array.isArray(animJson.images)) {
                    animJson.images = [newImgName];
                }
                animContent = JSON.stringify(animJson, null, 2);
            } catch (e) {
                console.error("Failed to parse Animation.json, exporting original unmodified", e);
            }
            files.push({
                name: animName,
                content: animContent
